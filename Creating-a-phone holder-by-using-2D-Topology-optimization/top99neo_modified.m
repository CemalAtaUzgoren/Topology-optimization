function top99neo_modified(nelx,nely,volfrac,penal,rmin,ft,ftBC,eta,beta,move,maxit,bc,F)
% ------------------ PRE. 1) MATERIAL AND CONTINUATION PARAMETERS
E0 = 1;                                                                    % Young modulus of solid
Emin = 1e-9;                                                               % Young modulus of "void"
nu = 0.3; minch=1e-2;                                                                % Poisson ratio
...
fixed = bc; %[1 : 1 : 2*( nely + 1 )]; %fixed = union( 1 : 2 : 2*( nely + 1 ), 2 * nodeNrs( end, end ) );          % restrained DOFs
[ pasS, pasV ] = deal( [], [] );                                           % UD, passive solid and void el.
% F_full=full(F);                % define load vector
...
while ch > minch && loop < maxit
  ...
  colormap( gray ); imagesc( reshape(1 - xPhys, nely, nelx ) );
